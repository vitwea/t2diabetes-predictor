"""
T2 Diabetes Predictor — Streamlit App
======================================
Run with:  streamlit run src/app/appweb.py
"""

import joblib
from src.pipeline.run_preprocessing import preprocessing, load_artifacts as load_pipeline_artifacts
import numpy as np
import pandas as pd
import shap
...
from sklearn.calibration import CalibratedClassifierCV  # required for unpickling
...
.stSlider [data-baseweb="slider"] { background: #1a2e44; }
div[data-testid="metric-container"] { background: #0c1827; border: 1px solid #1a2e44; border-radius: 12px; padding: 14px; }
</style>
""", unsafe_allow_html=True)

# ── Constants ─────────────────────────────────────────────────────────────────
# THRESHOLD is loaded dynamically from the model bundle (see load_artifacts)
...
@st.cache_resource
def load_artifacts():
    # appweb.py is in project root → .parent gives the root directly
    root = Path(__file__).resolve().parent

    bundle_path    = root / "models" / "final_diabetes_model.pkl"
    cal_path       = root / "models" / "HistGradientBoosting_calibrated.pkl"
    explainer_path = root / "models" / "shap_explainer.pkl"

    model, explainer = None, None
    threshold = 0.310

    # Priority 1: bundle saved by train.py → {"model": ..., "threshold": ..., "best_params": ...}
    if bundle_path.exists():
        bundle = joblib.load(bundle_path)
        if isinstance(bundle, dict):
            model     = bundle["model"]
            threshold = bundle.get("threshold", 0.310)
        else:
            model     = bundle
            threshold = 0.310

    # Priority 2: standalone calibrated model saved by notebook 06
...
model, explainer, THRESHOLD = load_artifacts()

# ── Preprocessing via pipeline (same logic used in training) ─────────────────
@st.cache_resource
def load_pipeline_artifacts_cached():
    root = Path(__file__).resolve().parent
    return load_pipeline_artifacts(root / "data" / "dataset" / "preprocessing_artifacts.joblib")

def preprocess_input(d: dict) -> pd.DataFrame:
    """Apply the same preprocessing pipeline used during training (transform mode)."""
    df_raw = pd.DataFrame([d])
    artifacts = load_pipeline_artifacts_cached()
    encoding_config = artifacts.get("encoding_info", {}).get("encoding_config", {})
...
def make_shap_chart(shap_df: pd.DataFrame) -> go.Figure:
    ...
    colors = ["#f07060" if v > 0 else "#00d4a0" for v in top["shap_value"]]
    fig = go.Figure(go.Bar(
        ...
        textfont={"size": 11, "color": "#cdd8e8", "family": "DM Mono"},
    ))
...
<div class="app-header">
    ...
    <p class="app-subtitle">
        Machine learning risk assessment · NHANES dataset (CDC) · HistGradientBoosting
    </p>
    <div style="margin-top: 12px;">
        <span class="badge">AUC 0.854</span>
        <span class="badge">Recall 86%</span>
        <span class="badge">SHAP Explainability</span>
        <span class="badge">28,452 samples</span>
    </div>
</div>
""", unsafe_allow_html=True)

if model is None:
    st.warning("⚠️ Model not found. Expected at `models/diabetes_final_model.pkl`. Run the training pipeline first.", icon="⚠️")
...
with st.sidebar:
    ...
    st.divider()
    predict_btn = st.button("🔍 Predict Risk", type="primary", width=True)
...
if predict_btn:
    if model is None:
        st.error("Model not loaded. Copy `t2diabetes_model.pkl` to the `model/` folder.")
    else:
        ...
        with col1:
            ...
            st.plotly_chart(make_gauge(prob, risk), width=True, config={"displayModeBar": False})
        ...
        if shap_df is not None:
            ...
            st.plotly_chart(make_shap_chart(shap_df), width=True, config={"displayModeBar": False})
            ...
            st.info("SHAP explainer not found. Run `python generate_shap_explainer.py` first to generate `models/shap_explainer.pkl`.")
        ...
        Predictions are probabilistic estimates based on population-level NHANES data. Always consult a qualified healthcare professional for medical decisions.
        </div>
        """, unsafe_allow_html=True)
    ...
    <div style="text-align:center; padding: 80px 40px; color: #4a6080;">
        ...
        <div style="font-size: 1.3rem; font-weight: 700; color: #cdd8e8; margin-bottom: 10px;">
